...
call_timers = {}
call_timers_ru = {}  # Separate timers for Russian calls if needed
...
def end_call_after_timeout(call_id, control_url, message):
                ...
                if control_url:
                    ...
                    timer_thread = threading.Thread(
                        ...
                        args=(call_id, control_url,
                              "Thank you for trying our demo agent. Your session has now ended. When you're ready, please use the booking section here on this page to schedule a consultation with one of our experts. We look forward to assisting you soon. Have a great day!")
                    )
...
# Russian webhook
@app.route('/server_ru', methods=['POST'])
def handle_call_events_ru():
    if not verify_signature(request):
        print("[Webhook RU] Signature verification failed!")
        return jsonify({"error": "Invalid signature"}), 401
    ...
    print(f"[Webhook RU] Received message type: {message_type}, call_id: {call_id}")

    if message_type == 'status-update':
        status = message_data.get('status')
        print(f"[Webhook RU] Call status update for {call_id}: {status}")

        if status == 'in-progress' and call_id:
            if call_id not in call_timers_ru:
                call_timers_ru[call_id] = time.time()
                control_url = message_data.get('call', {}).get('monitor', {}).get('controlUrl')

                if control_url:
                    print(f"[Webhook RU] Starting timer thread for call {call_id}")
                    timer_thread = threading.Thread(
                        target=end_call_after_timeout,
                        args=(call_id, control_url,
                              "Спасибо за использование нашего демонстрационного агента. Ваш сеанс завершён. Когда будете готовы, пожалуйста, используйте раздел бронирования на этой странице для записи консультации с одним из наших экспертов. Мы с нетерпением ждём возможности помочь вам. Хорошего дня!")
                    )
                    timer_thread.daemon = True
                    timer_thread.start()
                else:
                    print(f"[Webhook RU] No control URL found for call {call_id}")
            else:
                print(f"[Webhook RU] Timer already started for call {call_id}")
    else:
        print(f"[Webhook RU] Ignored message type: {message_type}")
